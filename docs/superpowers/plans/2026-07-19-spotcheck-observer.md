...
**Interfaces:**
- Consumes: Task 3 の `RawTable`、Task 4 の `ObserveStrategy`
- Produces: `export async function collectTables(page: Page): Promise<RawTable[]>`、`export async function applyDivisionFilter(page: Page, divisionLabel: string): Promise<boolean>`
...
Playwright を起動するテストは書かない（実サイトに接続するため）。ここでは盲検の検査だけを足す。
...
期待: `ENOENT: no such file or directory, open 'tools/spotcheck/observe.ts'` で失敗する（`observe.ts` は Task 6 で作る）。
...
- [ ] **Step 4: テストが失敗し続けることを確認する**

```bash
node --test tools/spotcheck/observeStrategy.test.ts
```

期待: 盲検の検査は `observe.ts` が無いため失敗したままである。他の 3 tests は pass する。Task 6 で `observe.ts` を作ると通る。

- [ ] **Step 5: コミットする**

```bash
git add packages/scraper/tools/spotcheck/observeStrategy.ts packages/scraper/tools/spotcheck/observeStrategy.test.ts
PATH="$PWD/node_modules/.bin:$PATH" git commit -m "feat(spotcheck): 表の収集と区分フィルタ操作を追加"
```
...
- Test: `packages/scraper/tools/spotcheck/observeStrategy.test.ts`（Task 5 の盲検検査がここで通る）

**Interfaces:**
- Consumes: Task 1-3 の `observeCore.ts`、Task 4-5 の `observeStrategy.ts`、`judgeReport.ts` の `PlanSample` / `ObservedSample`
- Produces: CLI。`test-results/_spotcheck/{raw,observed,screenshots}/` に書き出す

`plan.json` を読み、各サンプルについて `prepare` で到達し、戦略に応じて表を読み、`observed/<連番>.json` と `raw/<連番>.json` とスクリーンショットを書く。1 サンプルの失敗が全体を止めないよう、例外は `reached: false` に変換する。
...
```bash
git add packages/scraper/tools/spotcheck/observe.ts
PATH="$PWD/node_modules/.bin:$PATH" git commit -m "feat(spotcheck): 観測 CLI observe.ts を追加"
```
...
gh pr create --base master --title "feat(spotcheck): 観測を observe.ts に正式化し区分フィルタ型サイトに対応" --body "<Task 7 の検証結果を含む本文>"
```
