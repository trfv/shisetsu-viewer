...
- パイロット自治体: `tokyo-koutou`

## 背景と目的

`packages/scraper` の各自治体スクレイパーは、Playwright テストとして実装され、
CI（`.github/workflows/scraper.yml`、cron `0 4,16 * * *`）で定期実行される。
セレクタは XPath・ロール名・ステータス画像ファイル名にハードコードされており
（例: `tokyo-koutou/index.ts` の `//*[@id="disp"]/center/table[3]/...`、
`image/lw_emptybs.gif`）、**決定論的だが、サイトの DOM 変化に対して脆い**。
...
| A | 初期スコープ | **1自治体（`tokyo-koutou`）でパイロット** → 横展開。 |
...
- パイロットでは `tokyo-koutou/index.test.ts` の catch 節からこれを呼ぶ。
...
定期実行(cron) → scrape 失敗 → retry_scrape 失敗（同セレクタ再試行で救済されず）
  ...
  → 人間が Issue に気づく
  → 手元で `/repair-scraper tokyo-koutou` 起動（Max フラット枠）
...
- **エンドツーエンド検証（手動・受け入れ基準）**：`tokyo-koutou/index.ts` のセレクタを
