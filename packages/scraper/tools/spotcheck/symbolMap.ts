...
// 「空いていません」系も、AVAILABLE 側の「空いて」より先に潰す必要があるためここに置く。
const LABEL_CATEGORIES: readonly (readonly [RegExp, SlotCategory])[] = [
  [
    /空きなし|空枠なし|空いていません|空いてません|予約あり|予約済|予約不可|使用中|使用禁止|利用不可|利用中止|取消|音出し|抽選/,
    "UNAVAILABLE",
  ],
  // 「空いて」は大田区が img alt に「空いています」と書くため（実サイトの観測値）。
  [/一部空き|空き|空いて/, "AVAILABLE"],
