...
Cloudflare Workers にデプロイ済みのサーバーに接続します。API トークンが必要です。

#### API トークンの取得

管理者に API トークンの発行を依頼してください。
...
`YOUR_API_TOKEN` は https://app.shisetsudb.com のユーザーページで発行した API トークンに置き換えてください。
